...
const serviceHomepage = 'https://github.com/lblod/remote-login-service';
...
const ensureUser = async function(claims, graph) {
  ...
  if (queryResult.results.bindings.length) {
    ...
    return { personUri: result.person.value, personId: userId };
  } else {
...
const selectBestuurseenheid = async function(groupId) {
    const queryResult = await query(`
    PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
    PREFIX dcterms: <http://purl.org/dc/terms/>
    SELECT ?group
